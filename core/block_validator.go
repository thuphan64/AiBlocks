...
// VerifyUncles verifies the given block's uncles and applies the AiBlocks
...
func calcDifficultyDiehard(time, parentTime uint64, parentDiff *big.Int, diehardBlock *big.Int) *big.Int {
	// https://github.com/aiblocksproject/ECIPs/blob/master/ECIPS/ECIP-1010.md
	// algorithm:
	// diff = (parent_diff +
...
func calcDifficultyDefused(time, parentTime uint64, parentNumber, parentDiff *big.Int) *big.Int {
	// https://github.com/aiblocks/EIPs/blob/master/EIPS/eip-2.mediawiki
	// algorithm:
	// diff = (parent_diff +
	//         (parent_diff / 2048 * max(1 - (block_timestamp - parent_timestamp) // 10, -99))
	//        )
