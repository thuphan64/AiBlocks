...
func NewHeaderChain(chainDb ethdb.Database, config *ChainConfig, mux *event.TypeMux, getValidator getHeaderValidatorFn, procInterrupt func() bool) (*HeaderChain, error) {
	...
	if hc.genesisHeader == nil {
		...
		glog.V(logger.Info).Infof("Wrote default aiblocks %v genesis block", genname)
		glog.D(logger.Info).Infof("Wrote default aiblocks %v genesis block", logger.ColorGreen(genname))
