...
import (
	...
	"github.com/aiblocksproject/go-aiblocks/p2p/discover"
	"github.com/aiblocksproject/go-aiblocks/p2p/nat"
	"github.com/aiblocksproject/go-aiblocks/pow"
	"github.com/aiblocksproject/go-aiblocks/whisper"
...
var (
	...
	devModeDataDirPath = filepath.Join(os.TempDir(), "/aiblocks_dev_mode")
)
...
// --> <home>/<AiBlocksClassic>(defaulty) or --datadir
...
// --> <home>/<AiBlocksClassic>/<mainnet|testnet|custom-net>, per --chain
...
func MakeSystemNode(version string, ctx *cli.Context) *node.Node {
	...
	// Configure the AiBlocks service
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		glog.Fatalf("%v: failed to register the AiBlocks service: ", ErrStackFail, err)
	}
	if shhEnable {
...
func mustMakeEthConf(ctx *cli.Context, sconf *core.SufficientChainConfig) *eth.Config {
	...
	// Override any default configs in dev mode
	if ctx.GlobalBool(aliasableName(DevModeFlag.Name, ctx)) {
		// Override the AiBlocks protocol configs
