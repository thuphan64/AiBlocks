...
func status(ctx *cli.Context) error {
	...
	printme := []struct {
		...
		{"AiBlocks configuration", formatEthConfigPretty(ethConf)},
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node) *eth.AiBlocks {
	// Start up the node itself
	StartNode(stack)

	// Unlock any account specifically requested
	var aiblocks *eth.AiBlocks
	if err := stack.Service(&aiblocks); err != nil {
		glog.Fatal("aiblocks service not running: ", err)
	}
	...
	if ctx.GlobalBool(aliasableName(AddrTxIndexFlag.Name, ctx)) && ctx.GlobalBool(aliasableName(AddrTxIndexAutoBuildFlag.Name, ctx)) {
		a := aiblocks.BlockChain().GetAtxi()
		if a == nil {
			panic("somehow atxi did not get enabled in backend setup. this is not expected")
		}
		a.AutoMode = true
		go core.BuildAddrTxIndex(aiblocks.BlockChain(), aiblocks.ChainDb(), a.Db, math.MaxUint64, math.MaxUint64, 10000)
	}
	if ctx.GlobalBool(aliasableName(MiningEnabledFlag.Name, ctx)) {
		if err := aiblocks.StartMining(ctx.GlobalInt(aliasableName(MinerThreadsFlag.Name, ctx)), ctx.GlobalString(aliasableName(MiningGPUFlag.Name, ctx))); err != nil {
