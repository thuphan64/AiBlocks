...
// NewProtocolManager returns a new aiblocks sub protocol manager. The AiBlocks sub protocol manages peers capable
// with the aiblocks network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	// Unregister the peer from the downloader and AiBlocks peer set
...
func (pm *ProtocolManager) Stop() {
	glog.V(logger.Info).Infoln("Stopping aiblocks protocol handler...")
	...
	glog.V(logger.Info).Infoln("AiBlocks protocol handler stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	// Execute the AiBlocks handshake
...
// EthNodeInfo represents a short summary of the AiBlocks sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    int         `json:"network"`    // AiBlocks network ID (1=Mainnet, 2=Morden)
