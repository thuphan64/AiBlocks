// Copyright 2014 The go-aiblocks Authors
...
// Package eth implements the AiBlocks protocol.
package eth
...
import (
	...
	"github.com/aiblocksproject/go-aiblocks/common/httpclient"
	"github.com/aiblocksproject/go-aiblocks/common/registrar/ethreg"
	"github.com/aiblocksproject/go-aiblocks/core"
	"github.com/aiblocksproject/go-aiblocks/core/types"
	"github.com/aiblocksproject/go-aiblocks/eth/downloader"
	"github.com/aiblocksproject/go-aiblocks/eth/filters"
	...
	"github.com/aiblocksproject/go-aiblocks/node"
...
type AiBlocks struct {
	...
	// Channel for shutting down the aiblocks
...
func New(ctx *node.ServiceContext, config *Config) (*AiBlocks, error) {
	...
	if genesis == nil {
		...
		glog.V(logger.Info).Infof("Successfully wrote default aiblocks mainnet genesis block: %s", logger.ColorGreen(genesis.Hash().Hex()))
...
// APIs returns the collection of RPC services the aiblocks package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *AiBlocks) APIs() []rpc.API {
	return []rpc.API{
		{
			Namespace: "eth",
			Version:   "1.0",
			Service:   NewPublicAiBlocksAPI(s),
			Public:    true,
		}, {
			Namespace: "eth",
			Version:   "1.0",
...
func (s *AiBlocks) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *AiBlocks) Etherbase() (eb common.Address, err error) {
	eb = s.etherbase
	if eb.IsEmpty() {
...
func (self *AiBlocks) SetEtherbase(etherbase common.Address) {
	self.etherbase = etherbase
	self.miner.SetEtherbase(etherbase)
}

func (s *AiBlocks) StopMining()         { s.miner.Stop() }
func (s *AiBlocks) IsMining() bool      { return s.miner.Mining() }
func (s *AiBlocks) Miner() *miner.Miner { return s.miner }

func (s *AiBlocks) AccountManager() *accounts.Manager  { return s.accountManager }
func (s *AiBlocks) BlockChain() *core.BlockChain       { return s.blockchain }
func (s *AiBlocks) TxPool() *core.TxPool               { return s.txPool }
func (s *AiBlocks) EventMux() *event.TypeMux           { return s.eventMux }
func (s *AiBlocks) ChainDb() ethdb.Database            { return s.chainDb }
func (s *AiBlocks) DappDb() ethdb.Database             { return s.dappDb }
func (s *AiBlocks) IsListening() bool                  { return true } // Always listening
func (s *AiBlocks) EthVersion() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
func (s *AiBlocks) NetVersion() int                    { return s.netVersionId }
func (s *AiBlocks) ChainConfig() *core.ChainConfig     { return s.chainConfig }
func (s *AiBlocks) Downloader() *downloader.Downloader { return s.protocolManager.downloader }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *AiBlocks) Protocols() []p2p.Protocol {
	return s.protocolManager.SubProtocols
}

// Start implements node.Service, starting all internal goroutines needed by the
// AiBlocks protocol implementation.
func (s *AiBlocks) Start(srvr *p2p.Server) error {
	if s.AutoDAG {
		s.StartAutoDAG()
	}
...
func (s *AiBlocks) WaitForShutdown() {
	<-s.shutdownChan
}
...
// makedag
func (self *AiBlocks) StartAutoDAG() {
...
func (self *AiBlocks) StopAutoDAG() {
	if self.autodagquit != nil {
...
func (self *AiBlocks) HTTPClient() *httpclient.HTTPClient {
	return self.httpclient
}

func (self *AiBlocks) Solc() (*compiler.Solidity, error) {
	var err error
	if self.solc == nil {
...
// set in js console via admin interface or wrapper from cli flags
func (self *AiBlocks) SetSolc(solcPath string) (*compiler.Solidity, error) {
